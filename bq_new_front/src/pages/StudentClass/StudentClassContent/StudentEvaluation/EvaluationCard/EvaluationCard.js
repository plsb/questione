...
import { MoreVert, Edit, PlayArrow } from '@material-ui/icons';
...
const EvaluationCard = props => {
    const { className, history, refresh, setRefresh, evaluation, studentClassId, setTabValue, ...rest } = props;
    ...
    const onEdit = () => {
        history.push(`/student-class/${studentClassId}/evaluation-details/${evaluation.id}`);
    }
                ...
                action={
                    <div>
                        <Tooltip title="Realizar avaliação">
                            <IconButton
                                aria-label="settings"
                                onClick={() => history.push(`/code/${props.idApplication}`)}>
                                <PlayArrow />
                            </IconButton>
                        </Tooltip>
                    </div>
